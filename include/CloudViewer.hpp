...
class CloudViewer : public Processor<pcl::PointCloud<PointType>::ConstPtr> {
    typedef pcl::PointCloud<PointType> Cloud;
    typedef typename Cloud::ConstPtr CloudConstPtr;
    pcl::visualization::PCLVisualizer::Ptr cloud_viewer_;

public:
    CloudViewer(const std::shared_ptr<Buffer<CloudConstPtr>> cloud_buffer, size_t id, std::string description) :
            Processor<CloudConstPtr>(cloud_buffer, id, description) {
    }
